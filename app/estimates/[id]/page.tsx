...
interface Lead { id: string; first_name: string; last_name: string; phone?: string; email?: string; address?: string; city?: string; state?: string; zip?: string }
...
export default function EstimateBuilderPage() {
  ...
  // Data
  const [options,      setOptions]      = useState<Option[]>([{ option_num:1, option_label:'Option 1', sort_order:0, items:[{ section_name:'Scope of Work', description:'', is_exclusion:false, qty:1, unit_price:0, line_total:0, sort_order:0 }] }])
  ...
  async function loadAll() {
    ...
    const [estRes, optsRes, photosRes, templatesRes, attachRes, leadsRes, productsRes] = await Promise.all([
      ...
      supabase.from('leads').select('id, first_name, last_name, phone, email, metadata').order('created_at', { ascending: false }).limit(100),
  ...
  // Upload inspection photos (multiple)
  ...
  // Upload/replace document template file
  ...
  function handleDescriptionChange(optIdx: number, itemIdx: number, value: string) {
  ...
  async function save() {
    ...
    await supabase.from('estimates').upsert({
      ...
      property_photo_url: photoUrl, notes,
    ...
    for (const opt of options) {
      ...
      for (let i = 0; i < opt.items.length; i++) {
        ...
        await supabase.from('estimate_line_items').insert({ option_id: optId, estimate_id: estimateId, section_name: item.section_name, description: item.description, is_exclusion: item.is_exclusion, qty: item.qty, unit_price: item.unit_price, line_total: item.qty * item.unit_price, sort_order: i })
      }
    }
  ...
  function addOption() {
    ...
    setOptions(prev => [...prev, { option_num: num, option_label: `Option ${num}`, sort_order: prev.length, items: [{ section_name: 'Scope of Work', description: '', is_exclusion: false, qty: 1, unit_price: 0, line_total: 0, sort_order: 0 }] }])
  ...
  function addLineItem(optIdx: number) {
    setOptions(prev => {
      const next = [...prev]
      next[optIdx] = { ...next[optIdx], items: [...next[optIdx].items, { section_name: 'Scope of Work', description: '', is_exclusion: false, qty: 1, unit_price: 0, line_total: 0, sort_order: next[optIdx].items.length }] }
      return next
    })
  }
  ...
  function updateItem(optIdx: number, itemIdx: number, field: keyof LineItem, value: any) {
    setOptions(prev => {
      const next = [...prev]
      const items = [...next[optIdx].items]
      items[itemIdx] = { ...items[itemIdx], [field]: value, line_total: field === 'qty' ? value * items[itemIdx].unit_price : field === 'unit_price' ? items[itemIdx].qty * value : items[itemIdx].line_total }
      next[optIdx] = { ...next[optIdx], items }
      return next
    })
  }
  ...
  function optionTotal(optIdx: number) {
    return options[optIdx]?.items.filter(i => !i.is_exclusion).reduce((sum, i) => sum + (i.qty * i.unit_price), 0) || 0
  }
        ...
        <div className="max-w-3xl mx-auto p-6 space-y-8">
          ...
          <section id="section-cover" className="bg-white rounded-xl border border-gray-200 overflow-hidden">
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="border border-blue-100 bg-blue-50 rounded-lg p-3">
                <label className="text-xs font-medium text-blue-700 mb-1 block">Auto-fill from existing lead</label>
                <select value={leadId} onChange={e => selectLead(e.target.value)} className="w-full text-sm border border-blue-200 rounded-lg px-3 py-2 focus:outline-none focus:border-blue-400 bg-white">
                  <option value="">— Select a lead to auto-fill —</option>
                  {leads.map(l => <option key={l.id} value={l.id}>{l.first_name} {l.last_name}</option>)}
                </select>
                <p className="text-xs text-blue-500 mt-1">Client info will auto-fill but can still be edited below.</p>
              </div>
              ...
              <div className="border-t border-gray-100 pt-4">
                ...
                <div className="flex gap-3 items-start">
                  <label className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-gray-200 rounded-lg p-4 cursor-pointer hover:border-blue-400 hover:bg-blue-50 transition-colors">
                    ...
                    <span className="text-xs text-gray-500">{uploadingCover ? 'Uploading...' : 'Click to upload photo'}</span>
              ...
              {options[activeOption] && (
                <div>
                  ...
                  {options[activeOption].items.map((item, itemIdx) => (
                    <div key={itemIdx} className={`grid grid-cols-12 gap-2 mb-2 p-2 rounded-lg relative ${item.is_exclusion ? 'bg-orange-50 border border-orange-100' : 'bg-gray-50'}`}>
                      <div className="col-span-4 relative">
                        <input
                          value={item.section_name}
                          onChange={e => handleDescriptionChange(activeOption, itemIdx, e.target.value)}
                        ...
                        {activeSuggestionFor === `${activeOption}-${itemIdx}` && productSuggestions.length > 0 && (
                          <div className="absolute top-full left-0 right-0 z-20 bg-white border border-gray-200 rounded-lg shadow-lg mt-1 overflow-hidden">
                            {productSuggestions.map(p => (
                              <button key={p.id} onMouseDown={() => applyProduct(activeOption, itemIdx, p)} className="w-full text-left px-3 py-2 text-xs hover:bg-blue-50 border-b border-gray-50 last:border-none">
                                <div className="font-medium text-gray-800">{p.name}</div>
                                <div className="text-gray-400 truncate">{p.description?.slice(0,60)}...</div>
                              </button>
                            ))}
                          </div>
                        )}
                      ...
                      <div className="col-span-3">
                        <textarea value={item.description} onChange={e => updateItem(activeOption, itemIdx, 'description', e.target.value)} rows={2} placeholder="Description..." className="w-full text-xs border border-gray-200 rounded px-2 py-1 focus:outline-none resize-none bg-white" />
                      </div>
                      <div className="col-span-1">
                        <input type="number" value={item.qty} onChange={e => updateItem(activeOption, itemIdx, 'qty', parseFloat(e.target.value)||0)} className="w-full text-xs border border-gray-200 rounded px-2 py-1 focus:outline-none bg-white text-right" />
                      </div>
                      <div className="col-span-2">
                        <input type="number" value={item.unit_price} onChange={e => updateItem(activeOption, itemIdx, 'unit_price', parseFloat(e.target.value)||0)} className="w-full text-xs border border-gray-200 rounded px-2 py-1 focus:outline-none bg-white text-right" />
                      </div>
                      <div className="col-span-1 text-xs text-right font-medium text-gray-700 flex items-center justify-end">{fmt(item.qty * item.unit_price)}</div>
                      <div className="col-span-1 flex items-center justify-center">
                        <button onClick={() => removeItem(activeOption, itemIdx)} className="text-red-400 hover:text-red-600 text-lg leading-none">×</button>
                      </div>
                    </div>
                  ))}
          ...
          {showDocs && (
            <section id="section-docs" className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              ...
              <div className="p-5">
                <p className="text-xs text-gray-500 mb-4">Toggle documents to include. Upload new PDF to replace any outdated file.</p>
                  ...
                  {docTemplates.map(t => (
                    <div key={t.id} className={`p-3 rounded-lg border transition-colors ${t.is_enabled ? 'border-blue-200 bg-blue-50' : 'border-gray-200 bg-white'}`}>
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        {t.file_url && <a href={t.file_url} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-600 hover:underline">View current PDF</a>}
          ...
          {showFinancing && (
            <section id="section-financing" className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              ...
              <div className="p-5">
                <p className="text-xs text-gray-500 mb-4">Select financing partners to show. Informational only — no auto-calculation.</p>
                <div className="grid grid-cols-2 gap-4">
          ...
          {showTc && (
            <section id="section-tc" className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              ...
              <div className="p-5">
                <p className="text-xs text-gray-500 mb-3">Upload a PDF for each state/client type. The PDF will be included when printing.</p>
                ...
                {!tcFileUrl && (
                  <div className="bg-gray-50 rounded-lg p-4 text-xs text-gray-500 border border-gray-200">
                    <p className="font-medium mb-1">No PDF uploaded yet.</p>
                    <p>Upload your T&C PDF above. Until then, the default Elite Work T&C text will show in the preview.</p>
                  </div>
                )}
              </div>
            </section>
          ...
          {customPages.map((cp, i) => (
            <section key={i} id={`section-custom-${i}`} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              <div className="bg-gray-50 border-b border-gray-200 px-5 py-3 flex items-center gap-2">
                ...
                <input
                  value={cp.title}
                  onChange={e => setCustomPages(prev => { const n=[...prev]; n[i]={...n[i],title:e.target.value}; return n })}
                  className="font-semibold text-gray-800 bg-transparent border-none focus:outline-none focus:bg-gray-100 rounded px-1 flex-1"
                />
              ...
              <div className="p-5">
                {cp.type === 'pdf' ? (
                  ...
                  <textarea
                    value={cp.content}
                    onChange={e => setCustomPages(prev => { const n=[...prev]; n[i]={...n[i],content:e.target.value}; return n })}
                    rows={8}
                    placeholder="Type your custom page content here..."
