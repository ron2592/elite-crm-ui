...
import { Search, RefreshCw, Upload, Download, X, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function ContactsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return leads.filter(l => {
      ...
      const matchSearch = !q || name.includes(q) || phone.includes(q) || city.includes(q) || email.includes(q);

      const matchStatus = !filterStatus || l.status === filterStatus;
      const matchSource = !filterSource || (l.lead_sources as any)?.id === filterSource;

      let matchDate = true;
      if (dateFrom || dateTo) {
        const received = new Date(l.created_at);
        if (dateFrom && received < new Date(dateFrom + "T00:00:00")) matchDate = false;
        if (dateTo   && received > new Date(dateTo   + "T23:59:59")) matchDate = false;
      }
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="relative">
          ...
          <input
            ...
            className="pl-8 pr-3 py-1.5 rounded-md border border-border bg-background text-sm focus:outline-none focus:ring-2 focus:ring-primary/40 w-56"
          />
        </div>

        {/* Status */}
        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)}
          className="rounded-md border border-border bg-background px-2 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-primary/40 text-muted-foreground">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                return (
                  <tr key={lead.id}
                    className={`border-b hover:bg-muted/20 cursor-pointer transition-colors ${isSelected ? "bg-primary/5" : i % 2 === 0 ? "" : "bg-muted/10"}`}
                  ...
                  >
                    ...
                    <td className="px-4 py-3 whitespace-nowrap">
                      {lsa ? (
                        <span className={`text-xs ${LSA_COLORS[lsa] || "text-muted-foreground"}`}>
                          {lsa.replace("_", " ")}
                        </span>
